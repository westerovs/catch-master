...
Такой элемент не нужно включать в адаптивный resize, пока его размер и вертикальное положение не зависят от viewport. 
Бейджи стартового экрана используют именно этот принцип.
...
- привязка справа — `optionsToggleBtn`;
- привязка слева — `StatBadge`.
