...
import {createRect} from '@/game/utils/commonUtils.ts'
import GameUtils from '@/game/utils/gameUtils/GameUtils.ts'
import type ScoreboardView from './ScoreboardView.ts'
...
export default class ScoreRow extends Container {
  ...
  #height = 80
  #rowWidth: number
  #baseTextStyle: TextStyleOptions = {
    ...primaryFontStyle,
    fill: 0xffffff,
    fontSize: 26,
  }

  constructor({view, id, title, rank, score = 0}: ScoreRowOptions) {
    super({label: `scoreRow:${id ?? 'unknown'}`})
  ...
  createFallBackTexture = () => {
    ...
    const avatar = new Sprite({label: 'scoreboardFallbackAvatar', texture: fallbackTexture})
  ...
  #createAvatarContainer = () => {
    const avatarContainer = new Container({label: 'scoreboardAvatarContainer'})
  ...
  #createTextRank = () => {
    const textRank = new Text({label: 'scoreboardRank', text: this.#rank, style: {...this.#baseTextStyle}})
  ...
  #createTextScore = () => {
    const textScore = new Text({label: 'scoreboardScore', text: this.#score, style: {...this.#baseTextStyle}})
