...
import {ASSETS_URL, GAME_NAMES, PLATFORM_ID} from '@/game/config/constants.ts'
import Locator from '@/game/engine/Locator'
import SdkManager from '@/game/engine/SdkManager.ts'
import type {SpriteWithMetadata} from '../../../utils/gameUtils/GameUtils.ts'
import GameUtils from '../../../utils/gameUtils/GameUtils.ts'

type GameLogoView = SpriteWithMetadata & {
  label: string
  updateAdaptive: () => void
  _customPosition?: {
    x?: number
...
export default class GameLogo {
  ...
  init = async (): Promise<void> => {
    if (!SdkManager.flags?.showLogo) return
