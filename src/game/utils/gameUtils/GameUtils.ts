...
import PurchaseError from '@/game/ui/common/purchaseError/PurchaseError.ts'
...
type SpriteWithMetadata = Sprite & {
  _initScale: number
  type?: string
}
...
export default class GameUtils {
  static popupEl: HTMLDivElement | null = null
  static popupTimeout: ReturnType<typeof setTimeout> | null = null

  static get isFirstLevel() {
    const {levelIndex, userLevel} = Locator.storage.playerData
    return levelIndex === 0 && userLevel === 1
  }

  static createSprite(
    textureSource: string | Texture,
    {label, name, anchorX = 0.5, anchorY = 0.5, scale = 1, interactive}: SpriteOptions = {},
  ): SpriteWithMetadata {
    ...
    if (!texture) {
      console.error(`[GameUtils]: texture ${textureSource} not found, using white texture`)
      texture = Texture.WHITE
    }

    const sprite = new Sprite(texture) as SpriteWithMetadata
  ...
  static createCheckbox = ({
    ...
    const checkboxContainer = new Container({label: name ?? 'checkbox'})
    if (interactive) applyInteractive(checkboxContainer)

    const checkboxText = new Text({label: 'checkboxText', text, style})
  ...
  static createText(text: string | number, {name, style = {}, anchorX = 0.5, anchorY = 0.5}: TextOptions = {}) {
    const textElement = new Text({text, style})
    textElement.anchor.set(anchorX, anchorY)
    textElement.label = name ?? 'text'

    return textElement
  }

  static getPositionIndex(parent: Container, view: Container) {
    console.log(parent.children)
    return parent.children.indexOf(view)
  }

  static getLocalPosition(targetA: Container, targetB: Container) {
    const targetParent = targetB.parent
    if (!targetParent) {
      console.error('[GameUtils]: target object is not in the scene, returning zero position')
      return {x: 0, y: 0}
    }

    const globalCenter = targetA.toGlobal({x: targetA.width / 2, y: 0})
    const localPos = targetParent.toLocal(globalCenter)
  ...
  static isStringTrue = (value: unknown) => value === true || value === 'true'
  ...
  static showError = (err: unknown, {message = `${i18next.t('purchaseError')}`}: {message?: string} = {}) => {
    ...
    console.error('[GameUtils]: error dialog displayed', err)
  }
  ...
  static showTextPreloadAttempts = async (preloadText: Text | null, attempts: number, maxAttempts: number, err?: unknown) => {
    ...
    if (attempts >= maxAttempts) {
      ...
      console.error('[GameUtils]: loading failed, page will be reloaded')
    ...
    console.error(`[GameUtils]: loading failed, attempt ${attempts + 1} of ${maxAttempts}`, err)

    if (attempts < maxAttempts) await new Promise((res) => setTimeout(res, delay))
  }

  static formatKeysToLowerCase = (obj: Record<string, unknown>) => {
    return Object.fromEntries(Object.entries(obj).map(([key, value]) => [key.toLowerCase(), value]))
  }

  static checkLoadTime = (startTime: number) => {
    const endTime = performance.now()
    return Math.trunc(endTime - startTime)
  }
  ...
  static showPopUp = (text: string | number = '', {color = 'green', duration = 1000}: {color?: string; duration?: number} = {}) => {
...
const createGradientTexture = ({
  ...
  if (!ctx) {
    console.error('[GameUtils]: failed to get 2D context, using white texture')
...
export type {SpriteWithMetadata}
