...
type DebugRectOptions = ShapeOptions & {
  lineWidth?: number
}
...
const createRect = ({x = 0, y = 0, w = 0, h = 0, r = 0, color = 0xfff333, alpha = 1, center = false}: ShapeOptions) => {
...
const createCircle = ({x = 0, y = 0, r = 50, color = 0xfff333, alpha = 1}: ShapeOptions) => {
  const circle = new Graphics().circle(x, y, r / 2).fill({color, alpha})

  return circle
}

const shuffleArr = <T>(arr: readonly T[]) => {
